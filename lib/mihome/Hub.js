...
var WaterSensor  = require(__dirname + '/Sensors/WaterSensor');
...
function Hub(options) {
        ...
        if (!sensor) {
            ...
            try {
                if (options.browse) {
                    ...
                    sensor = this.sensorFactory(msg.sid, msg.model, rinfo.address);
                }
            }
            catch (e) {
    ...
    this.getKey = function (ip) {
        ...
        var key = this.keys[ip] || this.key;
        var cipher = crypto.createCipheriv('aes-128-cbc', key, this.iv);
        var crypted = cipher.update(this.token[ip], 'ascii', 'hex');
    ...
    this.sensorFactory = function (sid, model, ip) {
        if (this._state === 'CLOSED') return false;
        var sensor = null;
        switch (model) {
