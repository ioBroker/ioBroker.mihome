...
function Hub(options) {
    ...
    this.protoVer = ['1','0','6']; // Oldest protocol version
    ...
    this.protoMajor = function () {
        return this.protoVer[0];
    };
    ...
    this.onMessage = function (msgBuffer, rinfo) {
        ...
        try {
            ...
            if (msg.model === 'gateway') { // Issue with missing leading zero in gateway's sid
                msg.sid = msg.sid.padStart(12, '0');
            }
            if (msg.proto_version) {
                this.protoVer = msg.proto_version.split('.');
            }
            if (msg.params && (this.protoMajor() === '2')) {
